                ...
                <div class="card-body">
                    <div class="row mt-2 mb-3">
                        ...
                        <div class="col-md-6 text-md-right">
                            <a href="{{ route('individual.settings.bannerIndex') }}" class="btn btn-primary" type="button">
                                Back to all banners
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="col-md-12 ml-0">
                        <form action="{{ route('individual.settings.bannerUpdate',$banner->id) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            {{-- <input type="hidden" name="news_id" value="{{ $singleNews->id }}"> --}}
                            @include('individual.siteSetting.bannerCommonform')
...
@push('scripts')
    ...
    <script>
        @if (isset($banner->banner_image))
            var importedBaseImage = "{{ $banner->banner_image->path }}";
